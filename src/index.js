...
class UI extends React.Component {
  state = { hash: hashFromUrl() || "", amount: 30, currentShow: 1 };
  setHash = (e) => this.setState({ hash: e.target.value });  
  showMore = () => {
    if(this.state.amount <=100)
      this.setState({ 
        amount: this.state.amount * 2,
        currentShow: this.state.currentShow+1
      });      
  }
  ...
  render() {
    ...
    const displayed = getDisplayedPrevious(previous, this.state.currentShow)
...
function getDisplayedPrevious (previous, currentShow) {
  const displayed_before = previous.slice((currentShow-1)-30 , (currentShow-1) * 30)
  const displayed_next = previous.slice((currentShow - 1) * 30, currentShow * 30)
  console.log('displayed_before = ',displayed_before)
  console.log('displayed_next =', displayed_next)  
  return displayed_before, displayed_next
}
